# Roadmap

The following are intentionally outside the Build Week MVP unless the core vertical workflow is already green:

- Authentication and RBAC
- Multi-company tenancy
- PostgreSQL and migration framework
- Payments
- ERP, CRM, and procurement integrations
- n8n core dependency
- OCR
- Autonomous agents
- Generalized RAG
- Comprehensive AI Act policies or compliance claims
- Sophisticated PDF generation
- Enterprise PKI or KMS integration
- Mobile clients
- True incremental re-analysis or assessment merging
- Sophisticated retries, backoff, or response streaming
- Online deployment if it threatens the local DEMO path

Possible post-MVP work includes independently trusted timestamps, key rotation and revocation, richer evaluation sets, domain packs beyond vendor approval, and production observability.
