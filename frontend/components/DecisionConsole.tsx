...
type DemoSnapshot = {
  ...
  provenance: {
    ...
    live_assessment: {
      ...
      post_validation_transformations: Array<{
    ...
    human_approval: {
      source: string;
      human_entered_fields: string[];
      identity_assurance: string;
    };
    aelitium_outputs: { source: string; artifacts: string[] };
    limitations: string[];
  };
  trust_notice: string;
};
...
type Receipt = {
  decision_content: {
    model_execution: {
      ...
      assessment_source: "precomputed_fixture" | "gpt_generated_live";
      runtime_model_call: boolean;
      provider: string;
      model: string;
    };
...
type ReceiptResponse = {
  ...
  provenance: {
    ...
    human_approval: { source: string; approval_id: string };
    policy_result: { source: string };
    hashes: { source: string };
    signature: { source: string };
    receipt: { source: string };
    source_document_authenticity_verified: boolean;
  };
};
...
function requireDemoSnapshotProvenance(value: unknown): DemoSnapshot {
  if (!isRecord(value) || !isRecord(value.provenance)) {
    throw new Error(
      "DEMO API response is missing the B2 provenance contract. Restart the FastAPI backend and reload the page.",
    );
  }
  const demo = value.provenance.demo_assessment;
  const live = value.provenance.live_assessment;
  if (
    !isRecord(demo)
    || demo.assessment_source !== "precomputed_fixture"
    || demo.runtime_model_call !== false
    || !isRecord(live)
    || live.assessment_source !== "gpt_generated_live"
    || live.runtime_model_call !== true
    || !Array.isArray(live.post_validation_transformations)
  ) {
    throw new Error(
      "DEMO API response has incompatible provenance metadata. Restart the FastAPI backend and reload the page.",
    );
  }
      ...
      <div className="content-grid">
        <div className="primary-column">
          <article className="panel assessment-summary">
            <div className="panel-heading">
              ...
              <span className="model-chip">DEMO FIXTURE · NO MODEL CALL</span>
            </div>
            <p className="summary-copy">{snapshot.assessment.case_summary}</p>
            <div className="evidence-note">
              <span>i</span>
              <p>
                <strong>Assessment provenance</strong>
                This screen uses a deterministic derivation of the checked-in T2 fixture. No GPT request runs in DEMO.
                {" "}
        ...
        <aside className="secondary-column">
          ...
          <article className="panel documents-panel">
            ...
            <p className="form-footnote">These are checked-in, fictional Build Week fixtures—not authenticated external originals. AELITIUM commits their hashes but does not verify their authenticity.</p>
          </article>
        </aside>
      </div>
    ...
    <section className="screen-section">
      ...
      <div className="approval-grid">
        ...
        <form className="panel approval-form" onSubmit={onSubmit}>
          ...
          <p className="form-footnote">Human-entered data is limited to the declared name, condition text, and justification. AELITIUM binds the policy-selected role, decision type, identifiers, hashes, and timestamp.</p>
        </form>
      </div>
    </section>
  );
...
function ReceiptView({
  ...
  const assessmentProvenance = receipt.decision_content.model_execution;
    ...
    <section className="screen-section receipt-screen">
      <div className="screen-header">
        <div>
          ...
          <p>Three-part envelope: deterministic content, signed issuance metadata, detached signature. This DEMO receipt binds a precomputed fixture assessment—not a runtime GPT call.</p>
        </div>
        ...
        <div>
          <article className="panel receipt-card">
            ...
            <dl className="receipt-fields">
              ...
              <div><dt>Assessment source</dt><dd>{assessmentProvenance.assessment_source.replaceAll("_", " ")}<small>{assessmentProvenance.runtime_model_call ? assessmentProvenance.model : "no runtime model call"}</small></dd></div>
            </dl>
          ...
          <div className="trust-notice">
            <span>i</span>
            <p>{trustNotice} AELITIUM generated the policy result, canonical hashes, local signature, receipt, and verification result; the declared human approval remains a separate recorded input.</p>
          </div>
        </div>
